...
/*   Updated: 2025/11/19 19:39:19 by agerbaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
export class	pongRepository {
	...
	async addPongGame(pongAddDto: pongAddDto): Promise<number> {
		return new Promise((resolve, reject) => {
			...
			const	elements: number[] = [pongAddDto.getWinner(), pongAddDto.getP1(), pongAddDto.getP1Score(), pongAddDto.getP2(), pongAddDto.getP2Score(), pongAddDto.getStart()];
		...
		return new Promise((resolve, reject) => {
			...
			this.db.get(query, elements, (err: unknown, row: unknown) => {
				...
				if (!row) {
					...
					return reject(new Error(`The user ${gameId} doesn't exist`));
				}
