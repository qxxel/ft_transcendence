...
/*   Updated: 2025/12/06 20:49:17 by mreynaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
async function	recreatedToken(request: FastifyRequest, reply: FastifyReply): Promise<FastifyReply> {
	try {
		...
		const	resUser: AxiosResponse = await jwtAxios.get(`https://user:3000/${oldUser.id}`, { withCredentials: true, headers: { Cookie: request.headers.cookie || "" } });
