...
export async function	gatewayUserController(gatewayFastify: FastifyInstance): Promise<void> {
	...
	gatewayFastify.get('/lookup/:identifier', async (request: FastifyRequest, reply: FastifyReply) => {
		...
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.get(`http://user:3000/lookup/${identifier}`);
	...
	gatewayFastify.delete('/me', async (request: FastifyRequest, reply: FastifyReply) => {
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.delete(`http://user:3000/me`,
				{ headers: { 'user-id': userId } }
			);
	...
	gatewayFastify.patch('/stats/me', async (request: FastifyRequest, reply: FastifyReply) => {
		try {
			const	userId: AxiosHeaderValue = await getValidUserId(request);

			const	response: AxiosResponse = await gatewayAxios.patch(`http://user:3000/stats/me`, request.body,
				{ headers: { 'user-id': userId } }
			);
	...
	gatewayFastify.get('/friends/me', async (request: FastifyRequest, reply: FastifyReply) => {		
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.get(`http://user:3000/friends/me`,
				{ headers: { 'user-id': userId } }
			);
	...
	gatewayFastify.get('/friends/:id', async (request: FastifyRequest, reply: FastifyReply) => {		
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.get(`http://user:3000/friends/${parseId}`);
	...
	gatewayFastify.post('/friends/block/:targetId', async (request: FastifyRequest, reply: FastifyReply) => {
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.post(`http://user:3000/friends/block/${parseTargetId}`, request.body,
				{ headers: { 'user-id': userId } }
			);
	...
	gatewayFastify.delete('/friends/unblock/:targetId', async (request: FastifyRequest, reply: FastifyReply) => {
		try {
			...
			const	response: AxiosResponse = await gatewayAxios.delete(`http://user:3000/friends/unblock/${parseTargetId}`,
				{ headers: { 'user-id': userId } }
			);
